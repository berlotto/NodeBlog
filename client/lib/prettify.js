(function () {
    ...
    (function () {
        ...
        var Rc = (function () {
            var b = [fa, ga, ha, ia, ja, ka, x, la, ma, na, oa, pa, qa, ra, sa, ta, ua, va, Da, Ea, Fa, Ga, z, Ha, Ia, Ja, Ka, La, Ma, B, Na, Oa, Pa, Qa, Ra, Sa, Ta, Ua, Va, Wa, Xa, Ya, Za, $a, ab, bb, cb, db, eb, fb, gb, hb, ib, jb, kb, lb, mb, nb, ob, pb], a = qb;
            for (var d = 0; d < b.length; ++d) {
                var c = b[d];
                a += Qc(c.charAt(0)) ?
                    rb + c : $a + c.replace(/([^=<>:&])/g, sb)
            }
            a += tb;
            return new RegExp(a)
        })(), wa = /&/g, xa = /</g, ya = />/g, Sc = /\"/g;
        ...
        var Uc = /&lt;/g, Vc = /&gt;/g, Wc = /&apos;/g, Xc = /&quot;/g, Yc = /&amp;/g, Zc = /&nbsp;/g;

        function $c(b) {
            var a = b.indexOf(x);
            if (a < 0)return b;
            for (--a; (a = b.indexOf(yb, a + 1)) >= 0;) {
                var d = b.indexOf(Ga, a);
                if (d >= 0) {
                    var c = b.substring(a + 3, d), g = 10;
                    if (c && c.charAt(0) === zb) {
                        c =
                            c.substring(1);
                        g = 16
        ...
        var cd = /(?:[^<]+|<!--[\s\S]*?--\>|<!\[CDATA\[([\s\S]*?)\]\]>|<\/?[a-zA-Z][^>]*>|<)/g, dd = /^<!--/, ed = /^<\[CDATA\[/, fd = /^<br\b/i, Aa = /^<(\/?)([a-zA-Z]+)/;

        function gd(b) {
            var a = b.match(cd), d = [], c = 0, g = [];
            ...
            return{source: d.join(H), tags: g}
        }

        function hd(b) {
            return!!b.replace(/\s(\w+)\s*=\s*(?:\"([^\"]*)\"|'([^\']*)'|(\S+))/g,
                Jb).match(/[cC][lL][aA][sS][sS]=\"[^\"]*\bnocode\b/)
        }

        function aa(b, a, d, c) {
            if (!!a) {
                var g = d.call({}, a);
                if (b)for (var e = g.length; (e -= 2) >= 0;)g[e] += b;
                c.push.apply(c, g)
            }
        }
        ...
        function J(b, a) {
            ...
            var c = a.length, g = /\S/, e = function (k, f) {
                f = f || 0;
                var h = [f, I], q = H, i = 0, o = k;
                while (o.length) {
                    var r, l = j, m, p = d[o.charAt(0)];
                    if (p) {
                        m = o.match(p[1]);
                        l = m[0];
                        r = p[0]
                    ...
                    if (t) {
                        var A = m[1], v = l.indexOf(A), E = v + A.length, F = r.substring(5);
        ...
        function kd(b) {
            ...
            for (var d = 0; d < a.length; d += 2)if (a[d + 1] === M) {
                ...
                var e = b.substring(c, g), k = e.match(jd);
                if (k)a.splice(d,
                    2, c, T, c + k[1].length, M, c + k[1].length + (k[2] || H).length, T)
            }
            return a
        }
        ...
        function md(b, a) {
            for (var d = 0; d < a.length; d += 2) {
                ...
                if (c === T) {
                    ...
                    var k = b.substring(g, e), f = ld(k, g);
                    Q(f, a, d, 2);
                    d += f.length - 2
                }
            }
        ...
        function y(b) {
            var a = [], d = [];
            if (b.tripleQuotedStrings)a.push([W, /^(?:\'\'\'(?:[^\'\\]|\\[\s\S]|\'{1,2}(?=[^\']))*(?:\'\'\'|$)|\"\"\"(?:[^\"\\]|\\[\s\S]|\"{1,2}(?=[^\"]))*(?:\"\"\"|$)|\'(?:[^\\\']|\\[\s\S])*(?:\'|$)|\"(?:[^\\\"]|\\[\s\S])*(?:\"|$))/,
            ...
            b.regexLiterals && d.push([W, /^\/(?=[^\/*])(?:[^\/\x5B\x5C]|\x5C[\s\S]|\x5B(?:[^\x5C\x5D]|\x5C[\s\S])*(?:\x5D|$))+(?:\/|$)/,
                Rc]);
            var c = K(b.keywords), g = J(a, d), e = J([], [
                [I, /^\s+/, j, Sb],
                [I, /^[a-z_$@][a-z_$@0-9]*/i, j],
                [X, /^0x[a-f0-9]+[a-z]/i, j],
                ...
                for (var q = 0; q < h.length; q += 2) {
                    ...
                    if (i === I) {
                        ...
                        for (var p = 0, s = m.length; p < s; p += 2) {
                            var u = m[p + 1];
                            if (u === I) {
                                var t = m[p], A = p + 2 < s ? m[p + 2] : l.length, v = f.substring(t, A);
                                if (v === Ub)m[p + 1] = S; else if (v in c)m[p +
                                    1] = Vb; else if (/^@?[A-Z][A-Z$]*[a-z][A-Za-z$]*$/.test(v))m[p + 1] = v.charAt(0) === Ta ? X : Wb
                            }
                        }
                        Q(m, h, q, 2);
                        q += m.length - 2
        ...
        var Ba = y({keywords: Xb, hashComments: n, cStyleComments: n, multiLineStrings: n, regexLiterals: n});
            ...
            for (var c = 0; c < a.length; c += 2) {
                ...
                if (g === V) {
                    ...
                    if (d) {
                        ...
                        k = c + 2 < a.length ? a[c + 2] : b.length;
                        var f = b.substring(e,
                            k), h = f.length, q = h >= 2 && /^[\"\']/.test(f) && f.charAt(0) === f.charAt(h - 1), i, o, r;
                        if (q) {
                            o = e + 1;
                            r = k - 1;
                            i = f
        ...
        function pd(b, a, d) {
            var c = [], g = 0, e = j, k = j, f = 0, h = 0, q = bd(window.PR_TAB_WIDTH), i = /([\r\n ]) /g, o = /(^| ) /gm, r = /\r\n?|\n/g, l = /[ \r\n]$/, m = n;
        ...
        function w(b, a) {
            for (var d = a.length; --d >= 0;) {
                var c = a[d];
                if (G.hasOwnProperty(c))bc in window && console.log(cc, c); else G[c] = b
            }
        }
        ...
        w(y({keywords: qc, hashComments: n, cStyleComments: n}), [rc]);
        w(y({keywords: sc, cStyleComments: n}), [tc]);
        w(y({keywords: uc, hashComments: n, multiLineStrings: n}), [vc, wc, xc]);
        w(y({keywords: yc, hashComments: n, multiLineStrings: n, tripleQuotedStrings: n}), [zc, Ac]);
        w(y({keywords: Bc, hashComments: n, multiLineStrings: n, regexLiterals: n}), [Cc, Dc, Ec]);
        w(y({keywords: Fc, hashComments: n, multiLineStrings: n, regexLiterals: n}), [Gc]);
        w(y({keywords: Hc, cStyleComments: n, regexLiterals: n}),
            [Ic]);
        function Ca(b, a) {
            try {
                var d = gd(b), c = d.source, g = d.tags;
        ...
        function qd(b) {
            var a = window._pr_isIE6(), d = [document.getElementsByTagName(Jc), document.getElementsByTagName(Kc), document.getElementsByTagName(Lc)], c = [];
            for (var g = 0; g < d.length; ++g)for (var e = 0, k = d[g].length; e < k; ++e)c.push(d[g][e]);
            var f = 0;

            function h() {
                var q = window.PR_SHOULD_USE_CONTINUATION ? (new Date).getTime() +
                    250 : Infinity;
                            ...
                            if (za(i)) {
                                ...
                                for (var u = 0; u < i.attributes.length; ++u) {
                                    ...
                                    if (t.specified) {
                                        var A =
                                            t.name.toLowerCase();
                                        if (A === Nc)s.className = t.value; else s.setAttribute(t.name, t.value)
                                    }
                                }
                                s.innerHTML = p;
                ...
                if (f < c.length)setTimeout(h, 250); else b && b()
            }
        ...
        window.PR = {createSimpleLexer: J, registerLangHandler: w, sourceDecorator: y,
